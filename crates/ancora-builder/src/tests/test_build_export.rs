...
fn build_and_export_valid_graph() {
    ...
    let b = canvas.place_node("verifier.json_schema", "Verifier", Position::new(300.0, 0.0));

    edges
        .add_edge(a.clone(), "agent.llm", b.clone(), "verifier.json_schema", EdgeType::DataFlow)
        .expect("edge should be accepted");

    let spec = export_spec("build_export_test", &canvas, &edges, &ExportOptions::default())
    ...
    assert!(report.is_valid(), "exported spec should be valid: {:?}", report.diagnostics);
}
...
fn export_multi_node_graph() {
    ...
    edges.add_edge(a.clone(), "control.router", b.clone(), "agent.llm", EdgeType::DataFlow).unwrap();
    edges.add_edge(a.clone(), "control.router", c.clone(), "agent.llm", EdgeType::DataFlow).unwrap();
    edges.add_edge(b.clone(), "agent.llm", d.clone(), "control.merge", EdgeType::DataFlow).unwrap();
    edges.add_edge(c.clone(), "agent.llm", d.clone(), "control.merge", EdgeType::DataFlow).unwrap();
