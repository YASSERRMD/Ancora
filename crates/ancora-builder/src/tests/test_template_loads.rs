...
fn all_templates_load() {
    ...
    assert!(
        !registry.is_empty(),
        "template registry should not be empty"
    );
    assert!(
        registry.len() >= 4,
        "expected at least 4 built-in templates"
    );
}
...
fn single_agent_template_loads() {
    ...
    let t = registry
        .get("single_agent")
        .expect("single_agent template should exist");
...
fn rag_pipeline_template_loads() {
    ...
    let t = registry
        .get("rag_pipeline")
        .expect("rag_pipeline template should exist");
...
fn template_instantiation_renames_spec() {
    let registry = TemplateRegistry::default_registry();
    let spec = registry
        .instantiate("rag_pipeline", "my_custom_rag")
        .unwrap();
...
fn instantiated_spec_is_valid() {
    let registry = TemplateRegistry::default_registry();
    let spec = registry
        .instantiate("agent_verifier", "my_agent_verifier")
        .unwrap();
...
fn instantiated_spec_can_be_run() {
    let registry = TemplateRegistry::default_registry();
    let spec = registry
        .instantiate("single_agent", "run_template_test")
        .unwrap();
...
fn template_search_works() {
    ...
    assert!(
        !results.is_empty(),
        "search for 'rag' should return results"
    );
}
...
fn multi_agent_template_has_multiple_agents() {
    ...
    let t = registry
        .get("multi_agent")
        .expect("multi_agent template should exist");
    let agent_count = t
        .spec
        .nodes
        .iter()
        .filter(|n| n.kind.starts_with("agent."))
        .count();
    assert!(
        agent_count >= 2,
        "multi_agent template should have at least 2 agents"
    );
}
