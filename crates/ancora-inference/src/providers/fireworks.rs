...
pub fn build_fireworks_profile() -> ProviderProfile {
    ProviderProfile::new(
        ...
        AuthStrategy::BearerToken {
            env_var: "FIREWORKS_API_KEY".to_owned(),
        },
    )
    // Llama 3.1 family
    .add_model(
...
mod tests {
    ...
    fn fireworks_llama_70b_has_tools() {
        let p = build_fireworks_profile();
        let m = p
            .model_meta("accounts/fireworks/models/llama-v3p1-70b-instruct")
            .unwrap();
    ...
    fn fireworks_llama_8b_has_no_tools() {
        let p = build_fireworks_profile();
        let m = p
            .model_meta("accounts/fireworks/models/llama-v3p1-8b-instruct")
            .unwrap();
        assert!(!m.capabilities.tools);
    }
    ...
    fn fireworks_llama_large_context_window() {
        let p = build_fireworks_profile();
        let m = p
            .model_meta("accounts/fireworks/models/llama-v3p1-405b-instruct")
            .unwrap();
    ...
    fn fireworks_qwen_has_tools() {
        let p = build_fireworks_profile();
        let m = p
            .model_meta("accounts/fireworks/models/qwen2p5-72b-instruct")
            .unwrap();
        assert!(m.capabilities.tools);
    }
