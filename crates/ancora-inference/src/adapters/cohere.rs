...
pub fn parse_sse_line(line: &str) -> Option<crate::types::TokenEvent> {
    ...
    match evt {
        CohereStreamEvent::TextGeneration { text } => Some(crate::types::TokenEvent {
            text,
            finished: false,
        }),
        CohereStreamEvent::StreamEnd { .. } => Some(crate::types::TokenEvent {
            text: String::new(),
            finished: true,
        }),
...
mod tests {
    ...
    fn cohere_tool_call_fixture_parsed() {
        let resp = client()
            .parse_response(FIXTURE_TOOL, "command-r-plus")
            .unwrap();
    ...
    fn cohere_streaming_fixture_ordered() {
        let texts: Vec<String> = FIXTURE_STREAM
            .iter()
    ...
    fn cohere_streaming_combined_text() {
        let combined: String = FIXTURE_STREAM
            .iter()
            .filter_map(|l| parse_sse_line(l))
            .filter(|ev| !ev.text.is_empty())
            .map(|ev| ev.text)
