...
fn create_and_free_runtime_does_not_panic() {
    let rt = unsafe { ancora_create_runtime() };
    assert!(!rt.is_null());
...
fn free_null_runtime_is_noop() {
    unsafe { ancora_free_runtime(std::ptr::null_mut()) };
}
...
fn run_id_new_with_null_returns_null() {
    let id = unsafe { ancora_run_id_new(std::ptr::null()) };
    assert!(id.is_null());
}

#[test]
fn free_null_run_id_is_noop() {
    unsafe { ancora_run_id_free(std::ptr::null_mut()) };
}
