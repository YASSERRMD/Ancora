...
fn event_kind(event: &str) -> &str {
    if event.contains("started") {
        ...
        "completed"
    } else if event.contains("resumed") {
        "resumed"
    } else {
        event
    }
...
fn single_agent_scenario_produces_started_and_completed_events() {
    ...
    assert!(
        ...
        "single-agent: missing started event, got: {events:?}"
    );
    assert!(
...
fn human_in_loop_scenario_resume_produces_resumed_event() {
    ...
    assert_eq!(
        code,
        AncorErrorCode::Ok,
        "human-in-loop: resume must return Ok"
    );
    let events = drain_events(rt, &id);
    assert!(
...
fn crash_and_recover_scenario_events_are_deterministic() {
    ...
    assert_eq!(
        kinds1, kinds2,
        "crash-and-recover: event kinds must be deterministic across runs"
    );
}
...
fn conformance_scenario_single_agent_id_matches_expected() {
    ...
    assert_eq!(
        scenario.id, "single-agent",
        "single-agent scenario id must be 'single-agent'"
    );
}
...
fn conformance_scenario_count_is_four() {
    assert_eq!(
        conformance::all_scenarios().len(),
        4,
        "there must be exactly 4 conformance scenarios"
    );
}
...
fn conformance_single_agent_tag_contains_agent() {
    ...
    assert!(
        s.tags.contains(&"agent"),
        "single-agent scenario must have 'agent' tag"
    );
}
...
fn conformance_human_in_loop_tag_contains_suspend() {
    ...
    assert!(
        s.tags.contains(&"suspend"),
        "human-in-loop scenario must have 'suspend' tag"
    );
}
...
fn all_four_scenarios_produce_at_least_one_event_each() {
    ...
    for scenario in conformance::all_scenarios() {
        ...
        assert!(
            !events.is_empty(),
            "scenario '{}' must produce at least one event",
            scenario.id
        );
    }
    ancora_free_runtime(rt);
