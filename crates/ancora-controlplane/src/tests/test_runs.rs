...
mod tests {
    ...
    fn create_and_get_a_run() {
        ...
        let run = api.create(Some("test-token"), "tenant-1", RunPriority::Normal).unwrap();
    ...
    fn list_runs_with_tenant_filter() {
        let (mut store, auth) = setup();
        let mut api = RunsApi::new(&mut store, &auth);
        api.create(Some("test-token"), "tenant-1", RunPriority::Normal).unwrap();
        api.create(Some("test-token"), "tenant-2", RunPriority::Normal).unwrap();
        let page = api.list(Some("test-token"), Some("tenant-1"), None, None, 10).unwrap();
    ...
    fn cancel_and_resume_flow() {
        ...
        {
            let mut api = RunsApi::new(&mut store, &auth);
            let run = api.create(Some("test-token"), "t1", RunPriority::Normal).unwrap();
        ...
        store.resume_run(&run2.id, ResumeDecision { approved: true, reason: None }).unwrap();
    ...
    fn priority_ordering_honored() {
        let (mut store, auth) = setup();
        let mut api = RunsApi::new(&mut store, &auth);
        api.create(Some("test-token"), "t1", RunPriority::Low).unwrap();
        let high = api.create(Some("test-token"), "t1", RunPriority::Critical).unwrap();
        api.create(Some("test-token"), "t1", RunPriority::Normal).unwrap();
    ...
    fn pagination_cursor_works() {
        ...
        let page2 = api.list(Some("test-token"), None, None, page1.next_cursor.as_ref(), 3).unwrap();
