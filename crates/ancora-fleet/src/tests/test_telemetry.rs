...
fn test_health_telemetry_received() {
    ...
    let sample = HealthSample::new(id.clone()).with_metrics(30.0, 45.0, 60.0, 3600);
...
fn test_unhealthy_device_detected() {
    ...
    let mut sample = HealthSample::new(id.clone()).with_metrics(95.0, 80.0, 70.0, 100);
...
fn test_alert_level_critical() {
    ...
    let sample = HealthSample::new(id.clone()).with_metrics(98.0, 97.0, 50.0, 50);
...
fn test_telemetry_history() {
    ...
    for i in 0..5 {
        let s =
            HealthSample::new(id.clone()).with_metrics(i as f32 * 10.0, 20.0, 30.0, i as u64 * 60);
        telem.ingest(s);
    }
