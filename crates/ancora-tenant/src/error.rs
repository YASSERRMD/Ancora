...
pub enum TenantError {
    ...
    CrossTenantViolation { subject: String, resource_tenant: String },
}
...
impl fmt::Display for TenantError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            TenantError::QuotaExceeded { resource, used, max } =>
                write!(f, "{} quota exceeded: used={} max={}", resource, used, max),
            TenantError::CrossTenantViolation { subject, resource_tenant } =>
                write!(f, "cross-tenant violation: {} accessing {}", subject, resource_tenant),
        }
    }
}
