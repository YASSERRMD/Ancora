...
fn guard_assert_permission_ok() {
    ...
    assert!(guard.assert_permission("alice", "t1", &Permission::AgentWrite).is_ok());
}
#[test]
fn guard_assert_permission_err() {
    ...
    let policy = RolePolicy::new();
    let guard = RbacGuard::new(&store, &policy);
    assert!(guard.assert_permission("viewer", "t1", &Permission::AgentDelete).is_err());
}
#[test]
fn guard_minimum_role_ok() {
    ...
    let policy = RolePolicy::new();
    let guard = RbacGuard::new(&store, &policy);
    assert!(guard.assert_minimum_role("op", "t1", &Role::Developer).is_ok());
}
