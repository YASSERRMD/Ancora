...
impl SoftHsm {
    pub fn new() -> Self { Self { keys: HashMap::new(), next_handle: 1, operations: 0 } }

    pub fn generate_key(&mut self, slot_id: u32, label: impl Into<String>, algorithm: HsmKeyAlgorithm, tick: u64) -> u64 {
        ...
        self.keys.insert(handle, HsmKey::new(handle, slot_id, label, algorithm, class, tick));
        self.operations += 1;
        handle
    }

    pub fn get_key(&self, handle: u64) -> Option<&HsmKey> { self.keys.get(&handle) }

    pub fn delete_key(&mut self, handle: u64) -> bool {
        if self.keys.remove(&handle).is_some() { self.operations += 1; true } else { false }
    }
    ...
    pub fn encrypt(&self, handle: u64, data: &[u8]) -> Option<Vec<u8>> {
        if self.keys.contains_key(&handle) {
            ...
            for b in enc.iter_mut() { *b = b.wrapping_add(1); }
            Some(enc)
        } else {
            None
        }
    ...
    pub fn decrypt(&self, handle: u64, data: &[u8]) -> Option<Vec<u8>> {
        if self.keys.contains_key(&handle) {
            ...
            for b in dec.iter_mut() { *b = b.wrapping_sub(1); }
            Some(dec)
        } else {
            None
        }
    ...
    pub fn operation_count(&self) -> usize { self.operations }
    pub fn keys_for_slot(&self, slot_id: u32) -> Vec<&HsmKey> {
        self.keys.values().filter(|k| k.slot_id == slot_id).collect()
    }
}
