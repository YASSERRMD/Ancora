...
impl HsmStats {
    pub fn from_keys(keys: &[&HsmKey]) -> Self {
        ...
        for k in keys { *by_algorithm.entry(format!("{}", k.algorithm)).or_insert(0) += 1; }
        Self { total_keys, by_algorithm, extractable_count, sensitive_count }
    }

    pub fn sensitive_ratio(&self) -> f64 {
        if self.total_keys == 0 { return 0.0; }
