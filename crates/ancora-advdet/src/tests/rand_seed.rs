...
fn rand_seed_metrics_no_rng() {
    ...
    let results: Vec<f64> = (0..5)
        ...
        .collect();
    assert!(results.windows(2).all(|w| (w[0] - w[1]).abs() < f64::EPSILON));
}
...
fn rand_seed_fan_out_no_rng() {
    ...
    let runs: Vec<Vec<String>> = (0..5)
        .map(|_| fan_out("o", "a", inputs.clone(), "root").into_iter().map(|t| t.task_id).collect())
        .collect();
...
fn rand_seed_coordination_no_rng() {
    ...
    assert!(scores.windows(2).all(|w| (w[0] - w[1]).abs() < f64::EPSILON));
}
