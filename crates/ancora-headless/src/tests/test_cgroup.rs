use crate::cgroup::{apply_limits, CgroupApplyResult, CgroupLimits, CgroupUsage, presets};
...
fn test_cgroup_memory_max_value() {
    ...
    assert_eq!(limits.memory_max_value(), format!("{}", 1024u64 * 1024 * 1024));
}

#[test]
fn test_cgroup_validate_invalid_cpu() {
    let limits = CgroupLimits { cpu_quota_percent: 0, ..CgroupLimits::default() };
    assert!(limits.validate().is_err());
}

#[test]
fn test_cgroup_validate_invalid_memory() {
    let limits = CgroupLimits { memory_limit_mb: 16, ..CgroupLimits::default() };
    assert!(limits.validate().is_err());
}
...
fn test_cgroup_usage_within_limits() {
    let limits = CgroupLimits::default();
    let usage = CgroupUsage { cpu_usage_percent: 50.0, memory_used_mb: 1024, io_read_bytes: 0, io_write_bytes: 0 };
...
fn test_cgroup_usage_exceeds_memory() {
    ...
    let usage = CgroupUsage { cpu_usage_percent: 10.0, memory_used_mb: 1024, io_read_bytes: 0, io_write_bytes: 0 };
    assert!(usage.exceeds_memory(&limits));
}
