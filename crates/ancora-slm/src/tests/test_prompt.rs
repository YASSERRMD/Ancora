...
fn test_plain_format_contains_roles() {
    ...
    let opts = PromptOptions { style: PromptStyle::Plain, ..Default::default() };
    let out = format_prompt(&msgs, &opts);
    assert!(out.contains("System:"), "expected 'System:' in plain format");
...
fn test_alpaca_format_has_instruction_section() {
    ...
    let opts = PromptOptions { style: PromptStyle::Alpaca, ..Default::default() };
    let out = format_prompt(&msgs, &opts);
    assert!(out.contains("### Instruction:"), "expected Alpaca instruction section");
    assert!(out.contains("### Response:"), "expected Alpaca response section");
}
...
fn test_chatml_format_uses_im_tags() {
    ...
    let opts = PromptOptions { style: PromptStyle::ChatML, ..Default::default() };
    let out = format_prompt(&msgs, &opts);
    assert!(out.contains("<|im_start|>system"), "expected im_start for system");
    assert!(out.contains("<|im_start|>user"), "expected im_start for user");
    assert!(out.contains("<|im_start|>assistant"), "expected im_start for assistant turn");
}

#[test]
fn test_llama2_format_uses_inst_tags() {
    let msgs = vec![Message::user("What is Rust?")];
    let opts = PromptOptions { style: PromptStyle::Llama2Chat, ..Default::default() };
    let out = format_prompt(&msgs, &opts);
...
fn test_max_chars_truncation() {
    let msgs = vec![Message::user("This is a very long message that should be truncated.")];
    let opts = PromptOptions {
        style: PromptStyle::Plain,
        request_json: false,
        max_chars: Some(20),
...
fn test_slm_system_prompt_includes_steps() {
    ...
    assert!(prompt.contains("1. Read the input"), "expected numbered step 1");
