...
fn saml_valid_assertion_returns_token() {
    ...
    let assertion = SamlAssertion::new("assert-1", "https://saml-idp.example.com", "bob", "tenant-b", 0, 500)
        .with_signed(true);
...
fn saml_unsigned_assertion_rejected() {
    let (idp, config) = make_saml_setup();
    let assertion = SamlAssertion::new("assert-2", "https://saml-idp.example.com", "bob", "tenant-b", 0, 500);
...
fn saml_expired_assertion_rejected() {
    let (idp, config) = make_saml_setup();
    let assertion = SamlAssertion::new("assert-3", "https://saml-idp.example.com", "bob", "tenant-b", 0, 50)
        .with_signed(true);
    let err = idp.validate(&assertion, &config, 100).unwrap_err();
