...
fn stats_from_log_counts_correctly() {
    ...
    for _ in 0..4 { log.record(EvaluationRecord::from(1, &req, &PolicyDecision::Allow)); }
    for _ in 0..6 { log.record(EvaluationRecord::from(2, &req, &PolicyDecision::Deny("d".into()))); }
...
fn global_stats_covers_all_tenants() {
    ...
    log.record(EvaluationRecord::from(2, &req2, &PolicyDecision::Deny("d".into())));
