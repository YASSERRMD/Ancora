...
pub fn allow_https_only(tenant_id: impl Into<String>) -> NetworkPolicy {
    ...
    policy.add_rule(NetworkRule::new(
        "allow-https", "*", Some(443), Protocol::Tcp, Effect::Allow, 100,
    ).with_description("allow all outbound HTTPS"));
    policy
}

pub fn allow_internal_only(tenant_id: impl Into<String>, internal_domain: impl Into<String>) -> NetworkPolicy {
    let mut policy = NetworkPolicy::deny_by_default(tenant_id);
    let domain = format!("*.{}", internal_domain.into());
    policy.add_rule(NetworkRule::new(
        "allow-internal", domain, None, Protocol::Any, Effect::Allow, 100,
...
pub fn block_known_bad(policy: &mut NetworkPolicy, bad_host: impl Into<String>) {
    ...
    policy.add_rule(NetworkRule::new(
        id, host, None, Protocol::Any, Effect::Deny, 10,
