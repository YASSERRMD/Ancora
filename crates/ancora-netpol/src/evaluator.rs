        ...
        for rule in &policy.rules {
            if rule.matches(&req.destination_host, req.destination_port, &req.protocol) {
                return match rule.effect {
                    ...
                    Effect::Deny => PolicyDecision::Deny(format!("denied by rule '{}': {}", rule.id, rule.description)),
                };
            }
        }
        match policy.default_posture {
            DefaultPosture::AllowAll => PolicyDecision::Allow,
            DefaultPosture::DenyAll => PolicyDecision::Deny("denied by default posture".to_string()),
        }
    }
