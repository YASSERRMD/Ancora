...
impl LicenseBuilder {
    ...
    pub fn max_tenants(mut self, n: u32) -> Self {
        self.max_tenants = n;
        self
    }
    pub fn cap(mut self, c: EnterpriseCap) -> Self {
        self.caps.push(c);
        self
    }
    pub fn expires(mut self, tick: u64) -> Self {
        self.expires = Some(tick);
        self
    }

    pub fn build(self) -> EnterpriseLicense {
        let mut lic = EnterpriseLicense::new(
            self.id,
            self.tenant_id,
            self.tier,
            self.max_users,
...
impl IncidentBuilder {
    ...
    pub fn build(self) -> EnterpriseIncident {
        let mut inc = EnterpriseIncident::new(
            self.id,
            self.tenant_id,
            self.title,
            self.severity,
...
impl DomainScoreBuilder {
    ...
    pub fn findings(mut self, n: u32) -> Self {
        self.findings = n;
        self
    }
    pub fn critical(mut self, n: u32) -> Self {
        self.critical = n;
        self
    }
    pub fn build(self) -> DomainScore {
        DomainScore::new(self.domain, self.score, self.findings, self.critical)
    }
}
