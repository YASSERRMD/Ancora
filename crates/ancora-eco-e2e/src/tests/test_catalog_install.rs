...
fn test_catalog_install_adds_tool() {
    ...
    let found = catalog.get("search-tool").expect("tool must be retrievable");
...
fn test_catalog_duplicate_install_fails() {
    let mut catalog = ToolCatalog::new();
    catalog.install(CatalogTool::new("dup-tool", "desc", 2)).unwrap();
...
fn test_catalog_remove_tool() {
    let mut catalog = ToolCatalog::new();
    catalog.install(CatalogTool::new("rm-tool", "removable", 4)).unwrap();
...
fn test_catalog_list_sorted() {
    let mut catalog = ToolCatalog::new();
    catalog.install(CatalogTool::new("z-tool", "last", 5)).unwrap();
    catalog.install(CatalogTool::new("a-tool", "first", 6)).unwrap();
