...
fn test_trust_policy_blocks_low_trust_install() {
    ...
    let manifest = PluginManifest::new("untrusted-plugin", TrustLevel::Community, false, Some("abc123"));
...
fn test_trust_policy_allows_high_trust() {
    let policy = TrustPolicy::new(TrustLevel::Verified);
    let gate = TrustGate::new(policy);
    let manifest = PluginManifest::new("verified-plugin", TrustLevel::Official, true, Some("checksum"));
...
fn test_strict_policy_only_accepts_official() {
    ...
    gate.check(&official).expect("official must pass strict policy");
}
...
fn test_permissive_policy_accepts_community() {
    ...
    gate.check(&manifest).expect("community must pass permissive policy");
}
