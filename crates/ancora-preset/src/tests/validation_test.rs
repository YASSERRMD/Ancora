...
fn valid_preset_passes() {
    let preset =
        PresetDescriptor::new("test", "a valid preset").with_capability(Capability::Planning);
    assert!(validate(&preset).is_ok());
}

#[test]
fn empty_name_rejected() {
    let preset = PresetDescriptor::new("", "desc").with_capability(Capability::Planning);
...
fn empty_description_rejected() {
    let preset = PresetDescriptor::new("name", "").with_capability(Capability::Planning);
    let errs = validate(&preset).unwrap_err();
