...
pub fn security_runbook(incident_id: impl Into<String>) -> Runbook {
    ...
    rb.add_step(RunbookStep::new(
        "s1",
        "Isolate affected systems",
        "Revoke access tokens and network isolation",
    ));
    rb.add_step(RunbookStep::new(
        "s2",
        "Gather evidence",
        "Collect logs, screenshots, and artifacts",
    ));
    rb.add_step(RunbookStep::new(
        "s3",
        "Notify stakeholders",
    ...
    rb.add_step(RunbookStep::new(
        "s4",
        "Apply remediation",
        "Deploy patches or configuration fixes",
    ));
    rb.add_step(RunbookStep::new(
        "s5",
        "Verify fix",
        "Confirm systems are clean and monitoring",
    ));
    rb
}
