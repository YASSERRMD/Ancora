...
impl IncidentStats {
    pub fn for_tenant(incidents: &[&Incident], tenant_id: &str, current_tick: u64) -> Self {
        ...
        let mean_duration = if total == 0 { 0.0 } else { total_duration as f64 / total as f64 };
        Self { tenant_id: tenant_id.to_string(), total, active, resolved, by_severity, mean_duration }
    }
}
