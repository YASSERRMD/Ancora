...
fn random_graph_journal(run_id: &str, node_count: usize, seed: u64) -> Vec<JournalEvent> {
    ...
    for i in 0..node_count {
        let node_output = (seed
            .wrapping_mul((i as u64 + 1).wrapping_mul(6364136223846793005))
        ...
        events.push(JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: format!("node-{}", i),
                activity_kind: "compute".into(),
                input_json: format!(r#"{{"node":{},"seed":{}}}"#, i, seed),
                result_json: format!(r#"{{"out":{}}}"#, node_output),
                replayed: false,
            })),
        });
    }
...
#[test]
fn random_graphs_all_replay_to_completed() {
...
#[test]
fn random_graph_event_count_equals_nodes_plus_two() {
    for (nodes, seed) in GRAPH_SEEDS {
...
#[test]
fn random_graph_seq_is_monotonic_for_all_seeds() {
    for (nodes, seed) in GRAPH_SEEDS {
...
#[test]
fn different_seeds_produce_different_results_for_same_node_count() {
...
fn same_seed_produces_same_graph() {
    ...
    let keys1: Vec<String> = j1
        .iter()
        .filter_map(|e| {
            if let Some(Event::ActivityRecorded(a)) = &e.event {
                Some(a.result_json.clone())
            } else {
                None
            }
        })
        .collect();
    let keys2: Vec<String> = j2
        .iter()
        .filter_map(|e| {
            if let Some(Event::ActivityRecorded(a)) = &e.event {
                Some(a.result_json.clone())
