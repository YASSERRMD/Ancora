...
fn test_all_backends_have_conformance_test() {
    ...
    for backend in VECTOR_BACKENDS {
        assert!(
            covered.contains(backend),
            "no conformance test for backend: {backend}"
        );
    }
}
...
fn test_no_unknown_backend_in_conformance_map() {
    for (backend, _) in VECTOR_CONFORMANCE_TESTS {
        assert!(
            VECTOR_BACKENDS.contains(backend),
            "unknown backend in conformance map: {backend}"
        );
    }
}
...
fn test_weaviate_covered_by_dedicated_test() {
    let entry = VECTOR_CONFORMANCE_TESTS
        .iter()
        .find(|(b, _)| *b == "weaviate");
