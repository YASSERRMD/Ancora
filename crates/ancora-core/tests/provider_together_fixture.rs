...
fn together_llama3_8b_binding_resolves() {
    assert_eq!(together_router().resolve("llama-small", None), TOGETHER_LLAMA3_8B);
}

#[test]
fn together_mixtral_binding_resolves() {
    assert_eq!(together_router().resolve("mixtral-node", None), TOGETHER_MIXTRAL);
}
...
fn together_model_strings_have_prefix() {
    for m in [TOGETHER_LLAMA3_70B, TOGETHER_LLAMA3_8B, TOGETHER_MIXTRAL, TOGETHER_QWEN2] {
        assert!(m.starts_with("together_ai/"), "Expected together_ai/ prefix in {m}");
    }
}

#[test]
fn together_all_models_distinct() {
    let models = [TOGETHER_LLAMA3_70B, TOGETHER_LLAMA3_8B, TOGETHER_MIXTRAL, TOGETHER_QWEN2];
