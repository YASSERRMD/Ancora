...
fn make_metric_journal(run_id: &str, metric: &str, store: &str, result_json: &str) -> Vec<JournalEvent> {
    vec![
        JournalEvent {
        ...
        JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                ...
                input_json: format!(r#"{{"query":"metric test","top_k":2,"store":"{}","metric":"{}"}}"#, store, metric),
...
const COSINE_RESULT: &str = r#"[{"text":"cosine hit A","score":0.95},{"text":"cosine hit B","score":0.88}]"#;
const DOT_RESULT: &str = r#"[{"text":"dot hit A","score":28.4},{"text":"dot hit B","score":24.1}]"#;
const L2_RESULT: &str = r#"[{"text":"l2 hit A","distance":0.06},{"text":"l2 hit B","distance":0.11}]"#;

#[test]
fn cosine_metric_pgvector_replays() {
    let events = make_metric_journal("pg-cos", "cosine", "pgvector", COSINE_RESULT);
    let store = MemoryStore::new();
    for ev in &events { store.append("pg-cos", ev.clone()).unwrap(); }
    assert_eq!(replay_events("pg-cos", &store.read("pg-cos").unwrap()).unwrap().run.status, RunStatus::Completed);
}
...
fn dot_metric_milvus_replays() {
    ...
    let store = MemoryStore::new();
    for ev in &events { store.append("mv-dot", ev.clone()).unwrap(); }
    assert_eq!(replay_events("mv-dot", &store.read("mv-dot").unwrap()).unwrap().run.status, RunStatus::Completed);
}
...
fn l2_metric_lancedb_replays() {
    ...
    let store = MemoryStore::new();
    for ev in &events { store.append("ln-l2", ev.clone()).unwrap(); }
    assert_eq!(replay_events("ln-l2", &store.read("ln-l2").unwrap()).unwrap().run.status, RunStatus::Completed);
}
