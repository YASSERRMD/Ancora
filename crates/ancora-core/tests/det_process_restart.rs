...
fn checkpoint_journal(run_id: &str) -> Vec<JournalEvent> {
    vec![
        ...
        JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: "step-2".into(),
                activity_kind: "compute".into(),
                input_json: r#"{"n":20}"#.into(),
                result_json: r#"{"n":40}"#.into(),
                replayed: false,
            })),
        },
        JournalEvent {
...
fn simulate_restart_and_replay(run_id: &str) -> RunStatus {
    ...
    for ev in &persisted {
        store_after_restart.append(run_id, ev.clone()).unwrap();
    }
    replay_events(run_id, &store_after_restart.read(run_id).unwrap())
        .unwrap()
        .run
        .status
}
...
#[test]
fn replay_after_restart_produces_completed() {
    assert_eq!(
        simulate_restart_and_replay("det-rst-1"),
        RunStatus::Completed
    );
}

#[test]
fn replay_after_restart_is_idempotent_across_two_restarts() {
...
#[test]
fn journal_preserves_two_activity_events_across_restart() {
...
#[test]
fn journal_activity_results_stable_across_restart() {
...
#[test]
fn run_id_consistent_after_restart() {
    let j = checkpoint_journal("det-rst-id");
    for ev in &j {
        assert_eq!(ev.run_id, "det-rst-id");
    }
}
