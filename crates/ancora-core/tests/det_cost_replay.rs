...
struct ReplayCost { input_tokens: u64, output_tokens: u64, cost_usd: f64 }
    ...
    for (i, (inp, out)) in steps.iter().enumerate() {
        ...
        events.push(JournalEvent { event_id: format!("{}-{}", run_id, i+1), run_id: run_id.into(), seq: (i+1) as u64, recorded_at_ns: ((i+1)*1000) as i64,
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                ...
                result_json: format!(r#"{{"cost_usd":{:.10},"input_tokens":{},"output_tokens":{}}}"#, cost, inp, out),
                replayed: false })) });
    }
    let n = events.len();
...
#[test] fn cost_is_identical_on_two_replays() {
...
#[test] fn cost_formula_matches_direct_computation() {
...
#[test] fn cost_is_non_zero_for_non_empty_steps() {
    assert!(replay_total_cost(&build_cost_journal("det-cost-nz", STEPS)) > 0.0);
}

#[test] fn zero_tokens_produce_zero_cost() {
    assert_eq!(compute_cost(0, 0), 0.0);
}

#[test] fn cost_journal_event_count_matches_steps() {
