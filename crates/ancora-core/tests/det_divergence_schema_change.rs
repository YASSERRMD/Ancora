...
#[test]
fn old_schema_missing_new_required_field_is_incompatible() {
    let old_input = r#"{"query":"search","top_k":5}"#;
    let new_required = "embedding_model";
    assert!(
        !check_schema_compat(old_input, new_required),
        "old schema must be incompatible"
    );
}

#[test]
fn old_schema_with_new_required_field_is_compatible() {
...
#[test]
fn detect_divergence_catches_schema_driven_key_change() {
...
#[test]
fn detect_divergence_no_error_when_schema_unchanged() {
    let keys = vec!["retrieve:schema-v1".into(), "process:schema-v1".into()];
    assert!(detect_divergence(&keys, &keys).is_ok());
}

#[test]
fn schema_version_in_activity_key_enables_detection() {
...
#[test]
fn empty_schema_change_keys_both_empty_no_divergence() {
