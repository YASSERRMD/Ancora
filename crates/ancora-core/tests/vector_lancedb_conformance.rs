/// LanceDB conformance -- offline fixture, no live LanceDB.
use ancora_core::{
...
fn build_lancedb_journal(run_id: &str) -> Vec<JournalEvent> {
    vec![
        JournalEvent {
        ...
        JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: "retrieve-lancedb".into(),
                activity_kind: "retrieval".into(),
                input_json: r#"{"query":"lance docs","top_k":2,"table":"knowledge","uri":"./lance_data"}"#.into(),
                result_json: r#"[{"text":"LanceDB chunk A","score":0.95,"_distance":0.05},{"text":"LanceDB chunk B","score":0.88,"_distance":0.12}]"#.into(),
                replayed: false,
            })),
        },
        JournalEvent {
            ...
            event: Some(Event::RunCompleted(RunCompletedEvent {
                output_json: r#"{"answer":"lancedb-ok"}"#.into(),
            })),
        },
    ]
}
...
#[test]
fn lancedb_journal_replays_to_completed() {
    let store = MemoryStore::new();
    let run_id = "lance-run-151";
    for ev in &build_lancedb_journal(run_id) {
...
#[test]
fn lancedb_activity_key_contains_lancedb() {
...
#[test]
fn lancedb_input_has_uri_and_table() {
    let events = build_lancedb_journal("r");
    if let Some(Event::ActivityRecorded(a)) = &events[1].event {
...
#[test]
fn lancedb_result_has_distance_field() {
    let events = build_lancedb_journal("r");
    if let Some(Event::ActivityRecorded(a)) = &events[1].event {
...
#[test]
fn lancedb_result_has_two_chunks() {
    let events = build_lancedb_journal("r");
    if let Some(Event::ActivityRecorded(a)) = &events[1].event {
