...
impl SuiteSummary {
    fn passes(&self) -> bool {
        self.actual_tests >= self.min_tests
    }
}
...
fn test_all_suites_meet_minimum_threshold() {
    for s in SUITE_THRESHOLDS {
        assert!(
            s.passes(),
            "suite '{}' has {} tests but needs at least {}",
            s.suite,
            s.actual_tests,
            s.min_tests
        );
...
fn test_det_suite_at_least_100_tests() {
    let det = SUITE_THRESHOLDS
        .iter()
        .find(|s| s.suite == "det_suite")
        .unwrap();
...
fn test_sec_suite_at_least_50_tests() {
    let sec = SUITE_THRESHOLDS
        .iter()
        .find(|s| s.suite == "sec_suite")
        .unwrap();
