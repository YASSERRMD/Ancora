    ...
    for (i, key) in keys.iter().enumerate() {
        events.push(JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                ...
                activity_kind: "parallel-branch".into(),
                input_json: format!(r#"{{"branch":"{}"}}"#, key),
                result_json: format!(r#"{{"out":"{}"}}"#, key),
                replayed: false,
            })),
        });
    }
...
#[test]
fn parallel_join_produces_stable_key_order_when_sorted() {
...
#[test]
fn parallel_journal_event_count_equals_branches_plus_two() {
    let j = parallel_journal("det-par-cnt", BRANCHES);
    assert_eq!(j.len(), BRANCHES.len() + 2);
}

#[test]
fn parallel_join_all_branches_present() {
