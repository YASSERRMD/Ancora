...
fn build_time_journal(run_id: &str, ts_ns: i64, seed: u64) -> Vec<JournalEvent> {
    vec![
        ...
        JournalEvent {
            ...
            run_id: run_id.into(),
            seq: 1,
            recorded_at_ns: ts_ns,
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: "sampled".into(),
                activity_kind: "sampled".into(),
...
#[test]
fn journaled_timestamp_is_preserved_on_replay() {
    let j = build_time_journal("det-ts", FIXED_TS, FIXED_SEED);
    assert_eq!(
        j[1].recorded_at_ns, FIXED_TS,
        "timestamp must come from journal not live clock"
    );
}

#[test]
fn journaled_seed_produces_deterministic_result() {
...
#[test]
fn live_clock_is_not_used_on_replay() {
    let past_ts: i64 = 1_000_000_000;
    let j = build_time_journal("det-past-ts", past_ts, FIXED_SEED);
    assert_eq!(
        j[1].recorded_at_ns, past_ts,
        "replay must use journaled ts, not system clock"
    );
}

#[test]
fn timestamp_in_completed_event_is_after_activity() {
    let j = build_time_journal("det-ts-order", FIXED_TS, FIXED_SEED);
    assert!(
        j[2].recorded_at_ns > j[1].recorded_at_ns,
        "completed ts must be after activity ts"
    );
}
