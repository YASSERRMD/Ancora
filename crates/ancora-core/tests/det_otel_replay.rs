    ...
    vec![
        ...
        JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: "traced-step".into(),
                activity_kind: "traced".into(),
                input_json: format!(
                    r#"{{"trace_id":"{}","span_id":"0000000000000001","operation":"traced-step"}}"#,
                    trace_id
                ),
                result_json: format!(
                    r#"{{"trace_id":"{}","span_id":"0000000000000002","parent_span_id":"0000000000000001","status":"ok"}}"#,
                    trace_id
                ),
                replayed: false,
            })),
        ...
        JournalEvent {
            ...
            event: Some(Event::RunCompleted(RunCompletedEvent {
                output_json: r#"{"traced":true}"#.into(),
            })),
        },
    ]
}
...
#[test]
fn otel_trace_id_preserved_on_replay() {
    let j1 = build_otel_journal("det-otel-1", TRACE_ID);
    let j2 = build_otel_journal("det-otel-2", TRACE_ID);
    assert_eq!(
        get_span_field(&j1, "trace_id"),
        get_span_field(&j2, "trace_id")
    );
}

#[test]
fn otel_span_id_is_non_empty() {
