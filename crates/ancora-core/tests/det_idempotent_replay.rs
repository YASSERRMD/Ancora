    ...
    vec![
        ...
        JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: "idem-step".into(),
                activity_kind: "compute".into(),
                input_json: r#"{"x":10}"#.into(),
                result_json: r#"{"y":20}"#.into(),
                replayed: false,
            })),
        },
        JournalEvent {
...
fn replay_n_times(run_id: &str, n: usize) -> Vec<RunStatus> {
    ...
    (0..n)
        .map(|_| {
            ...
            replay_events(run_id, &store.read(run_id).unwrap())
                .unwrap()
                .run
                .status
        })
...
fn replay_does_not_mutate_journal() {
    ...
    assert_eq!(
        j_before.len(),
        j_after.len(),
        "replay must not add or remove journal events"
    );
}

#[test]
fn multiple_independent_replays_all_have_same_event_read_back() {
    let rid = "det-idem-ev";
...
#[test]
fn idempotent_replay_preserves_output_json() {
    ...
    if let Some(Event::RunCompleted(c)) = &j_after.last().unwrap().event {
        assert_eq!(
            c.output_json, expected_output,
            "output_json must not change after replay"
        );
    }
}
