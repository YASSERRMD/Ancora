...
fn test_all_envelopes_use_a2a_protocol() {
    for e in A2A_EXAMPLES {
        assert_eq!(
            e.protocol, A2A_PROTOCOL,
            "envelope for {}->{} has wrong protocol",
            e.sender_lang, e.recipient_lang
        );
    }
}
...
fn test_no_self_sender_recipient() {
    for e in A2A_EXAMPLES {
        assert_ne!(e.sender_lang, e.recipient_lang);
    }
}
