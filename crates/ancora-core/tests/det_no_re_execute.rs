...
#[test]
fn replayed_true_marks_activity_as_not_live() {
...
#[test]
fn replayed_false_marks_activity_as_live() {
...
#[test]
fn replayed_result_is_preserved_unchanged() {
    let expected = r#"{"text":"hi there"}"#;
    let j = build_journal_with_replayed_flag("det-nr-res", true);
    if let Some(Event::ActivityRecorded(a)) = &j[1].event {
        assert_eq!(
            a.result_json, expected,
            "result_json must not change on replay"
        );
    }
}

#[test]
fn replayed_activity_key_unchanged() {
...
#[test]
fn replayed_flag_does_not_alter_event_count() {
    assert_eq!(
