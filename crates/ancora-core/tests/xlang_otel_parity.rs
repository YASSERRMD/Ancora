...
fn make_lang_spans(lang: &'static str, base_span: &'static str) -> Vec<OtelSpan> {
    vec![
        OtelSpan { trace_id: TRACE_ID, span_id: base_span, parent_span_id: None, operation: "run", lang, status: "ok" },
        OtelSpan { trace_id: TRACE_ID, span_id: "000000000000000b", parent_span_id: Some(base_span), operation: "activity:main-agent", lang, status: "ok" },
    ]
}
...
fn root_span_has_no_parent() {
    for (lang, base) in SPANS {
        ...
        assert!(spans[0].parent_span_id.is_none(), "root span must have no parent for {}", lang);
    }
}
...
fn child_span_parent_is_root_span_id() {
    for (lang, base) in SPANS {
        let spans = make_lang_spans(lang, base);
        assert_eq!(spans[1].parent_span_id, Some(*base), "child parent mismatch for {}", lang);
    }
}
...
fn operations_match_across_languages() {
    let ops: Vec<Vec<&str>> = SPANS.iter().map(|(l, b)| make_lang_spans(l, b).iter().map(|s| s.operation).collect()).collect();
    let first = &ops[0];
