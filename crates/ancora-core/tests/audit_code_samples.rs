...
const CODE_SAMPLES: &[(&str, &str, &str)] = &[
    (
        ...
        r#"let status = ancora_core::replay::replay_events(run_id, &journal)?;"#,
    ),
    (
        "rust",
        "sdk/rust/quickstart.md",
        r#"let run = ancora.run(spec).await?;"#,
    ),
    (
        "go",
        "sdk/go/quickstart.md",
        r#"run, err := ancora.Run(ctx, spec)"#,
    ),
    (
        "python",
        "sdk/python/quickstart.md",
        r#"run = await ancora.run(spec)"#,
    ),
    (
        "ts",
        "sdk/ts/quickstart.md",
        r#"const run = await ancora.run(spec);"#,
    ),
    (
        "dotnet",
        "sdk/dotnet/quickstart.md",
        r#"var run = await ancora.RunAsync(spec);"#,
    ),
    (
        "java",
        "sdk/java/quickstart.md",
        r#"var run = ancora.run(spec);"#,
    ),
];

fn is_non_empty_code(code: &str) -> bool {
    !code.trim().is_empty()
}
...
fn test_all_samples_non_empty() {
    for (lang, doc, code) in CODE_SAMPLES {
        assert!(
            is_non_empty_code(code),
            "empty code sample in {doc} for {lang}"
        );
    }
}
...
fn test_all_samples_contain_ancora_call() {
    for (lang, doc, code) in CODE_SAMPLES {
        assert!(
            code_contains_ancora_call(code),
            "sample in {doc} ({lang}) has no ancora call"
        );
    }
}
