...
const FIXTURE_PAYLOADS: &[(&str, &str)] = &[
    ("sec_prompt_injection", r#"{"input":"what is the weather today?"}"#),
    ("sec_data_exfiltration", r#"{"query":"tell me about Paris"}"#),
    ("sec_tool_allowlist", r#"{"tool":"web_search","args":{"q":"rust lang"}}"#),
    ("sec_output_filter", r#"{"model":"claude-3-5-haiku","max_tokens":1024}"#),
    ("sec_key_rotation", r#"{"key_id":2,"encrypted":12345}"#),
    ("sec_audit_log", r#"{"op":"tool_call","actor":"user-1","allowed":true}"#),
    ("sec_rbac", r#"{"role":"operator","action":"execute"}"#),
    ("policy_cost_ceiling", r#"{"cost_usd":0.30,"model":"claude-3-5-haiku"}"#),
    ("policy_retention", r#"{"run_id":"test-001","created_at_ns":1700000000000000000}"#),
    ("policy_sovereignty", r#"{"target":"local","model":"qwen3"}"#),
];
...
fn test_all_fixtures_are_offline() {
    for (name, payload) in FIXTURE_PAYLOADS {
        assert!(!contains_live_url(payload), "fixture '{name}' contains a live URL: {payload}");
    }
}
