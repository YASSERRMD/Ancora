    ...
    for i in 0..length {
        ...
        events.push(JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: format!("tool:{}", tool),
                activity_kind: "tool-call".into(),
                input_json: format!(r#"{{"tool":"{}","step":{}}}"#, tool, i),
                result_json: format!(r#"{{"tool":"{}","value":{}}}"#, tool, result_val),
                replayed: false,
            })),
        });
    }
...
#[test]
fn all_tool_sequences_replay_to_completed() {
...
#[test]
fn tool_sequence_event_count_equals_length_plus_two() {
    for (len, seed) in SEQUENCES {
...
#[test]
fn same_seed_produces_same_tool_sequence() {
...
#[test]
fn all_tool_activity_keys_start_with_tool_prefix() {
    let j = tool_sequence_journal("det-tkeys", 6, 99);
    for ev in &j {
        if let Some(Event::ActivityRecorded(a)) = &ev.event {
            assert!(
                a.activity_key.starts_with("tool:"),
                "expected tool: prefix, got {}",
                a.activity_key
            );
        }
    }
...
#[test]
fn tool_sequence_seq_monotonic() {
