    ...
    vec![
        ...
        JournalEvent { event_id: format!("{}-1", run_id), run_id: run_id.into(), seq: 1, recorded_at_ns: 1_000,
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: "search".into(), activity_kind: "retrieval".into(),
                input_json: r#"{"q":"hello"}"#.into(),
                result_json: r#"{"docs":["doc1","doc2"]}"#.into(), replayed: false })) },
...
#[test] fn v1_journal_replays_to_completed_without_migration() {
    let store = MemoryStore::new();
    let rid = "det-v1-nomig";
    for ev in &v1_journal(rid) { store.append(rid, ev.clone()).unwrap(); }
    assert_eq!(replay_events(rid, &store.read(rid).unwrap()).unwrap().run.status, RunStatus::Completed);
}

#[test] fn v1_to_v2_migration_changes_q_to_query() {
    ...
    if let Some(Event::ActivityRecorded(a)) = &migrated[1].event {
        ...
        assert!(v.get("query").is_some(), "expected query field after migration");
        assert!(v.get("q").is_none(), "old q field must be removed");
    }
}

#[test] fn migrated_v1_journal_still_replays_to_completed() {
    let j = v1_journal("det-v2-mig");
    let migrated: Vec<JournalEvent> = j.iter().map(|e| migrate_v1_to_v2(e)).collect();
    let store = MemoryStore::new();
    let rid = "det-v2-mig";
    for ev in &migrated { store.append(rid, ev.clone()).unwrap(); }
    assert_eq!(replay_events(rid, &store.read(rid).unwrap()).unwrap().run.status, RunStatus::Completed);
}

#[test] fn migration_preserves_event_count() {
    let j = v1_journal("det-mig-cnt");
    let m: Vec<JournalEvent> = j.iter().map(|e| migrate_v1_to_v2(e)).collect();
    assert_eq!(j.len(), m.len());
}

#[test] fn migration_preserves_seq_numbers() {
    let j = v1_journal("det-mig-seq");
    let m: Vec<JournalEvent> = j.iter().map(|e| migrate_v1_to_v2(e)).collect();
    for (orig, mig) in j.iter().zip(m.iter()) { assert_eq!(orig.seq, mig.seq); }
}
