...
fn test_all_providers_have_doc_page() {
    ...
    for (provider, _) in PROVIDERS {
        assert!(
            doc_providers.contains(provider),
            "no doc page for provider: {provider}"
        );
    }
}
...
fn test_qwen_in_chinese_providers_doc() {
    ...
    assert!(qwen
        .map(|(_, doc)| doc.contains("chinese"))
        .unwrap_or(false));
}

#[test]
fn test_local_providers_documented() {
    let local_providers: Vec<&str> = PROVIDERS
        .iter()
...
fn test_all_doc_pages_end_with_md() {
    for (_, page) in PROVIDER_DOC_PAGES {
        assert!(page.ends_with(".md"), "not .md: {page}");
    }
}
