    ...
    vec![
        ...
        JournalEvent { event_id: format!("{}-1", run_id), run_id: run_id.into(), seq: 1, recorded_at_ns: 1_000,
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: "compute".into(), activity_kind: "deterministic".into(),
                input_json: input.into(), result_json: result.into(), replayed: false })) },
        JournalEvent { event_id: format!("{}-2", run_id), run_id: run_id.into(), seq: 2, recorded_at_ns: 2_000,
            event: Some(Event::RunCompleted(RunCompletedEvent { output_json: format!(r#"{{"result":"{}"}}"#, result) })) },
    ]
}
...
#[test] fn identical_inputs_produce_same_status_on_two_replays() {
    for i in 0..2 {
        let store = MemoryStore::new();
        let rid = format!("det-id-{}", i);
        for ev in &build_det_journal(&rid, INPUT_A, RESULT_A) { store.append(&rid, ev.clone()).unwrap(); }
        let state = replay_events(&rid, &store.read(&rid).unwrap()).unwrap();
        assert_eq!(state.run.status, RunStatus::Completed);
    }
}

#[test] fn identical_inputs_produce_same_event_count_on_two_replays() {
    let counts: Vec<usize> = (0..2).map(|i| {
...
#[test] fn identical_inputs_have_same_activity_result() {
    for i in 0..3 {
...
#[test] fn different_inputs_produce_different_results() {
...
#[test] fn replay_preserves_seq_numbers() {
