...
fn simulate_branch(branch_id: usize, seed: u64) -> BranchResult {
    let value = seed.wrapping_mul(6364136223846793005).wrapping_add(branch_id as u64);
    BranchResult { branch_id, value, ok: value % 17 != 0 }
}

fn join_results(results: &[BranchResult]) -> Option<u64> {
    if results.iter().all(|r| r.ok) {
        Some(results.iter().map(|r| r.value).fold(0u64, u64::wrapping_add))
    } else {
        None
    }
}
...
fn test_bench_10k_parallel_joins_under_200ms() {
    ...
    for i in 0..JOIN_BENCH_N {
        let branches: Vec<BranchResult> = (0..JOIN_BENCH_BRANCHES)
            ...
            .collect();
        if join_results(&branches).is_some() { success += 1; }
    }
    let elapsed = t0.elapsed().as_millis();
    assert!(elapsed < JOIN_BENCH_MS, "took {}ms budget {}ms", elapsed, JOIN_BENCH_MS);
    assert!(success > 0);
}
...
fn test_join_all_ok_returns_sum() {
    let results = vec![
        BranchResult { branch_id: 0, value: 10, ok: true },
        BranchResult { branch_id: 1, value: 20, ok: true },
    ];
...
fn test_join_one_failed_returns_none() {
    let results = vec![
        BranchResult { branch_id: 0, value: 10, ok: true },
        BranchResult { branch_id: 1, value: 20, ok: false },
    ];
