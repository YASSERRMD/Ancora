...
#[test] fn large_journal_replays_to_completed() {
    let store = MemoryStore::new();
    let rid = "det-large-perf";
    for ev in &large_journal(rid) { store.append(rid, ev.clone()).unwrap(); }
    let status = replay_events(rid, &store.read(rid).unwrap()).unwrap().run.status;
    assert_eq!(status, RunStatus::Completed);
}

#[test] fn large_journal_has_correct_event_count() {
    let j = large_journal("det-large-cnt");
    assert_eq!(j.len(), LARGE_N + 2);
}

#[test] fn large_journal_replay_completes_within_time_budget() {
    let store = MemoryStore::new();
...
#[test] fn large_journal_seq_monotonic() {
    let j = large_journal("det-large-seq");
    for (i, ev) in j.iter().enumerate() { assert_eq!(ev.seq, i as u64); }
}

#[test] fn large_journal_all_activity_keys_unique() {
    let j = large_journal("det-large-keys");
    let keys: std::collections::HashSet<String> = j.iter().filter_map(|e| {
