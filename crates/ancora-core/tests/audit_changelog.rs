...
const BREAKING_CHANGES: &[BreakingChange] = &[
    BreakingChange {
        version: "0.2.0",
        description: "journal seq field added",
        migration_documented: true,
    },
    BreakingChange {
        version: "0.3.0",
        description: "activity_key renamed from activity_name",
        migration_documented: true,
    },
    BreakingChange {
        version: "0.4.0",
        description: "HumanDecisionRequested options field added",
        migration_documented: true,
    },
    BreakingChange {
        version: "0.5.0",
        description: "cost model changed from flat to per-token",
        migration_documented: true,
    },
    BreakingChange {
        version: "0.6.0",
        description: "vector store config moved to VectorStoreConfig struct",
        migration_documented: true,
    },
];
...
fn test_all_breaking_changes_documented() {
    for change in BREAKING_CHANGES {
        assert!(
            change.migration_documented,
            "breaking change in {} not documented: {}",
...
fn test_no_undocumented_breaking_change() {
    let undoc: Vec<&BreakingChange> = BREAKING_CHANGES
        .iter()
        .filter(|c| !c.migration_documented)
        .collect();
