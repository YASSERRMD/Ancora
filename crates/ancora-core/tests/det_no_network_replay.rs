    ...
    vec![
        ...
        JournalEvent { event_id: format!("{}-1", run_id), run_id: run_id.into(), seq: 1, recorded_at_ns: 1_000,
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                activity_key: "api-call".into(), activity_kind: "llm".into(),
...
#[test] fn replayed_journal_has_no_live_network_calls() {
    let j = build_replayed_journal("det-net-1");
    assert_eq!(count_live_network_calls(&j), 0, "replayed activities must not trigger network calls");
}

#[test] fn replayed_activity_has_result_already_populated() {
    ...
    if let Some(Event::ActivityRecorded(a)) = &j[1].event {
        ...
        assert!(!a.result_json.is_empty(), "result_json must already be populated");
    }
}

#[test] fn non_replayed_activity_could_be_live() {
    let mut j = build_replayed_journal("det-net-3");
    if let Some(Event::ActivityRecorded(ref mut a)) = j[1].event {
        a.replayed = false;
    }
    assert_eq!(count_live_network_calls(&j), 1, "non-replayed activity with url is a potential live call");
}

#[test] fn url_in_input_flagged_as_potential_network() {
    let j = build_replayed_journal("det-net-4");
    if let Some(Event::ActivityRecorded(a)) = &j[1].event {
        assert!(a.input_json.contains("url"), "test fixture must have url in input");
    }
}

#[test] fn replayed_result_text_is_from_cache_not_live() {
    let j = build_replayed_journal("det-net-5");
    if let Some(Event::ActivityRecorded(a)) = &j[1].event {
        assert!(a.result_json.contains("cache"), "replayed result must come from journal cache");
    }
}
