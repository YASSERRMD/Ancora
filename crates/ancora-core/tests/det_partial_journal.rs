...
#[test]
fn partial_journal_only_started_is_not_completed() {
    let store = MemoryStore::new();
    let rid = "det-partial-0";
    for ev in &partial_journal(rid, 0) {
    ...
    assert_ne!(
        state.run.status,
        RunStatus::Completed,
        "partial journal must not be Completed"
    );
}

#[test]
fn partial_journal_two_steps_is_not_completed() {
    let store = MemoryStore::new();
    let rid = "det-partial-2";
    for ev in &partial_journal(rid, 2) {
        store.append(rid, ev.clone()).unwrap();
    }
    let state = replay_events(rid, &store.read(rid).unwrap()).unwrap();
...
#[test]
fn partial_journal_event_count_is_steps_plus_one() {
    assert_eq!(partial_journal("det-partial-cnt", 3).len(), 4);
}

#[test]
fn partial_journal_run_id_consistent() {
    let rid = "det-partial-id";
    for ev in &partial_journal(rid, 3) {
        assert_eq!(ev.run_id, rid);
    }
}

#[test]
fn partial_seq_monotonic() {
