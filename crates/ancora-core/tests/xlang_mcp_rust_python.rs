    ...
    vec![
        ...
        JournalEvent {
            ...
            event: Some(Event::ActivityRecorded(ActivityRecordedEvent {
                ...
                input_json:
                    r#"{"query":"xlang search","client_lang":"python","server_lang":"rust"}"#.into(),
                result_json: r#"[{"id":"r1","text":"Rust MCP result","server":"rust-mcp-server"}]"#
                    .into(),
                replayed: false,
            })),
        },
        JournalEvent {
            ...
            event: Some(Event::RunCompleted(RunCompletedEvent {
                output_json: r#"{"mcp_ok":true}"#.into(),
            })),
        },
    ]
}
...
#[test]
fn mcp_rust_server_journal_completes() {
    use ancora_core::{
    ...
    for ev in &build_mcp_journal(rid) {
...
#[test]
fn mcp_activity_key_contains_server_lang() {
...
#[test]
fn mcp_result_has_server_field() {
    let evs = build_mcp_journal("r");
    if let Some(Event::ActivityRecorded(a)) = &evs[1].event {
...
#[test]
fn mcp_activity_kind_is_tool_call() {
    let evs = build_mcp_journal("r");
    if let Some(Event::ActivityRecorded(a)) = &evs[1].event {
