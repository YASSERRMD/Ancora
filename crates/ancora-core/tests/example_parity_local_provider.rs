...
const LOCAL_RESULTS: &[LocalProviderResult] = &[
    LocalProviderResult { provider: "ollama", model: "llama3", base_url: "http://localhost:11434", local_only: true, response_prefix: "local:" },
    LocalProviderResult { provider: "lmstudio", model: "phi3", base_url: "http://localhost:1234", local_only: true, response_prefix: "local:" },
];
...
fn test_all_base_urls_are_localhost() {
    for r in LOCAL_RESULTS {
        assert!(r.base_url.contains("localhost"), "provider {} base_url is not localhost", r.provider);
    }
}
...
fn test_all_responses_have_local_prefix() {
    for r in LOCAL_RESULTS {
        assert!(r.response_prefix.starts_with("local"), "provider {} response prefix not local", r.provider);
    }
}
...
fn test_local_provider_ports_correct() {
    for (name, _, port) in LOCAL_PROVIDERS {
        if *name == "ollama" { assert_eq!(*port, 11434); }
        if *name == "lmstudio" { assert_eq!(*port, 1234); }
    }
}
...
fn test_no_remote_url_in_local_providers() {
    for r in LOCAL_RESULTS {
        assert!(!r.base_url.contains("api."), "provider {} has remote URL", r.provider);
    }
}
