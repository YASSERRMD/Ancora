...
fn sbom_to_csv_includes_header_and_component() {
    ...
    sbom.add_component(Component::new("c1", "openssl", "3.0", ComponentKind::Library, License::Apache2, "project", "sha"));
...
fn sbom_to_summary_contains_sbom_id_and_count() {
    let mut sbom = Sbom::new("s1", "t1", SbomFormat::CycloneDx, 0);
    sbom.add_component(Component::new("c1", "lib", "1.0", ComponentKind::Library, License::Mit, "vendor", "sha"));
