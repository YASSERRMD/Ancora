...
fn make_rollout_all_pass() -> crate::rollout::RolloutResult {
    let case = EvalCase { id: "c1".into(), input: "q".into(), expected: "a".into() };
...
fn make_rollout_half_pass() -> crate::rollout::RolloutResult {
    ...
    let case = EvalCase { id: "c2".into(), input: "q".into(), expected: "a".into() };
...
fn pass_power_k_all_pass() {
    ...
    assert!((ppk - 1.0).abs() < 1e-9, "all-pass -> pass^3 should be 1.0, got {}", ppk);
}
...
fn pass_power_k_half_pass() {
    ...
    // Expected: (0.5)^2 = 0.25
    assert!((ppk - 0.25).abs() < 0.01, "half-pass -> pass^2 ~ 0.25, got {}", ppk);
}
...
fn pass_at_k_all_pass() {
    ...
    assert!((pak - 1.0).abs() < 1e-9, "pass@3 should be 1.0 for all-pass, got {}", pak);
}
...
fn suite_pass_power_k_computed() {
    ...
    assert!((ppk - 0.625).abs() < 0.05, "suite pass^2 ~ 0.625, got {}", ppk);
}
...
fn suite_pass_at_k_computed() {
    ...
    assert!((pak - 1.0).abs() < 1e-9, "all-pass suite pass@2 = 1.0, got {}", pak);
}
