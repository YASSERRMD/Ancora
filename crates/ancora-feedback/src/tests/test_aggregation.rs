...
fn aggregation_accurate() {
    let feedback = vec![
        Feedback::new("f1", "r1", None, ThumbsRating::Up, Some("nice".into()), "alice", 0),
        Feedback::new("f2", "r1", None, ThumbsRating::Up, None, "bob", 1),
        Feedback::new("f3", "r1", None, ThumbsRating::Down, Some("bad".into()), "carol", 2),
    ...
    assert!((rate - 0.5).abs() < 1e-9, "Expected 50% approval rate, got {}", rate);
}
...
fn single_feedback_approval_rate_is_one() {
    let feedback = vec![
        Feedback::new("f1", "r1", None, ThumbsRating::Up, None, "user", 0),
    ];
    let metrics = aggregate(&feedback);
