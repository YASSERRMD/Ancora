use ancora_keymgmt::{
    aes256_encryption_key, ed25519_signing_key, ephemeral_key,
    ExpiryChecker, HsmConfig, HsmStub, KeyAlgorithm, KeyAuditEntry, KeyAuditLog,
    KeyBuilder, KeyOperation, KeyPurpose, KeyStats, KeyStore, KeyValidator,
    RotationPolicy, rotate_key,
};
...
fn main() {
    ...
    audit.record(KeyAuditEntry::new(100, "tenant-acme", "enc-key-1", 1, KeyOperation::Create, "admin", true));
    audit.record(KeyAuditEntry::new(101, "tenant-acme", "sig-key-1", 1, KeyOperation::Create, "admin", true));

    let policy = RotationPolicy::new(5).with_rotation_interval(1000);
    let enc_key = store.get_active("tenant-acme", "enc-key-1").unwrap();
    println!("Should rotate at tick 1100: {}", policy.should_rotate(enc_key, 1100));

    let new_ver = rotate_key(&mut store, "tenant-acme", "enc-key-1", "new-material-v2", 1100).unwrap();
    println!("Rotated enc-key-1 to version {}", new_ver);
    audit.record(KeyAuditEntry::new(1100, "tenant-acme", "enc-key-1", new_ver, KeyOperation::Rotate, "admin", true));
    ...
    let hsm_key = hsm.generate_key("hsm-key-1", "tenant-acme", KeyAlgorithm::EcdsaP256, KeyPurpose::Signing, 1200);
    ...
    println!("Rotations for enc-key-1: {}", audit.rotations_for("enc-key-1").len());
