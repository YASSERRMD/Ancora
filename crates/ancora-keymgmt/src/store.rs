...
impl KeyStore {
    ...
    pub fn get_active(&self, tenant_id: &str, id: &str) -> Result<&CryptoKey, KeyStoreError> {
        ...
        let versions = self.keys.get(&k).ok_or_else(|| KeyStoreError::KeyNotFound(id.to_string()))?;
        versions.iter().rev().find(|kv| kv.status == KeyStatus::Active)
            .ok_or_else(|| KeyStoreError::KeyNotActive(id.to_string()))
    }

    pub fn get_version(&self, tenant_id: &str, id: &str, version: u32) -> Result<&CryptoKey, KeyStoreError> {
        let k = format!("{tenant_id}:{id}");
        let versions = self.keys.get(&k).ok_or_else(|| KeyStoreError::KeyNotFound(id.to_string()))?;
        versions.iter().find(|kv| kv.version == version)
            .ok_or_else(|| KeyStoreError::KeyNotFound(format!("{id}@v{version}")))
    }

    pub fn get_latest_mut(&mut self, tenant_id: &str, id: &str) -> Result<&mut CryptoKey, KeyStoreError> {
        let k = format!("{tenant_id}:{id}");
        let versions = self.keys.get_mut(&k).ok_or_else(|| KeyStoreError::KeyNotFound(id.to_string()))?;
        versions.last_mut().ok_or_else(|| KeyStoreError::KeyNotFound(id.to_string()))
    }

    pub fn add_version(&mut self, tenant_id: &str, id: &str, new_key: CryptoKey) -> Result<(), KeyStoreError> {
        let k = format!("{tenant_id}:{id}");
        let versions = self.keys.get_mut(&k).ok_or_else(|| KeyStoreError::KeyNotFound(id.to_string()))?;
    ...
    pub fn list_tenant_active(&self, tenant_id: &str) -> Vec<&CryptoKey> {
        self.keys.values().flat_map(|v| v.iter()).filter(|k| k.tenant_id == tenant_id && k.is_active()).collect()
    }

    pub fn total_key_ids(&self) -> usize { self.keys.len() }
}
