use crate::{
    compromise_key, deactivate_key, destroy_key, CryptoKey, KeyAlgorithm, KeyAuditLog, KeyPurpose,
    KeyStatus, KeyStore,
};
...
fn deactivate_lifecycle_sets_inactive_and_records() {
    ...
    assert_eq!(
        store.get_latest_mut("t1", "k1").unwrap().status,
        KeyStatus::Inactive
    );
...
fn compromise_lifecycle_sets_compromised() {
    ...
    assert_eq!(
        store.get_latest_mut("t1", "k2").unwrap().status,
