...
impl KeyAuditLog {
    ...
    pub fn record(&mut self, entry: KeyAuditEntry) { self.entries.push_back(entry); }
    ...
    pub fn rotations_for(&self, key_id: &str) -> Vec<&KeyAuditEntry> {
        self.entries.iter().filter(|e| e.key_id == key_id && e.operation == KeyOperation::Rotate).collect()
    }

    pub fn all(&self) -> impl Iterator<Item = &KeyAuditEntry> { self.entries.iter() }
}
