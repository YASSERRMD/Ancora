...
fn sample_rubric() -> Rubric {
    Rubric::new()
        .add_criterion(Criterion::new("accuracy", "Is the answer factually correct?", 0.6))
        .add_criterion(Criterion::new("clarity", "Is the answer clearly stated?", 0.4))
}
...
fn test_llm_judge_offline_scores_perfect_match() {
    ...
    assert!(score.value > 0.8, "Expected high score, got {}", score.value);
}
...
fn test_llm_judge_empty_rubric() {
    ...
    assert!((score.value - 0.0).abs() < f64::EPSILON, "Empty rubric should score 0");
}

#[test]
fn test_llm_judge_custom_fn() {
    let rubric = Rubric::new()
        .add_criterion(Criterion::new("length", "Is the answer long enough?", 1.0));
    let g = LlmJudgeGrader::new(rubric, Box::new(|cand, _exp, _desc| {
        if cand.len() > 5 { 1.0 } else { 0.0 }
    }));
