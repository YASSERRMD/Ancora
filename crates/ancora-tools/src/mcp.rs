...
impl McpTransport for HttpTransport {
    fn send(&self, method: &str, params: serde_json::Value) -> Result<serde_json::Value, ToolError> {
...
impl McpClient {
    ...
    pub fn list_tools(&self) -> Result<Vec<McpToolDefinition>, ToolError> {
        ...
        tools.iter().map(|t| Ok(McpToolDefinition {
    ...
    pub fn call_tool(
        ...
        self.transport.send("tools/call", serde_json::json!({ "name": name, "arguments": input }))
    }
...
impl Tool for McpToolAdapter {
    ...
    fn input_schema(&self) -> serde_json::Value { self.definition.input_schema.clone() }
    fn effect(&self) -> ToolEffect { ToolEffect::Write }
...
mod tests {
    ...
    impl McpTransport for MockTransport {
        fn send(&self, method: &str, _params: serde_json::Value) -> Result<serde_json::Value, ToolError> {
            match method {
    ...
    fn mcp_tools_appear_and_invoke_through_the_contract() {
        ...
        let result = tools[0].call(&serde_json::json!({ "name": "world" })).unwrap();
