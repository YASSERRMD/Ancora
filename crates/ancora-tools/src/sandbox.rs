...
pub fn check_policy(
    ...
    if needs_network && !policy.allow_network {
        return Err(ToolError::ValidationFailed("network access denied by sandbox policy".into()));
    }
    if needs_fs_write && !policy.allow_filesystem_write {
        return Err(ToolError::ValidationFailed("filesystem write denied by sandbox policy".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    fn network_denied_by_default_policy() {
        let policy = SandboxPolicy { max_execution_ms: 1000, allow_network: false, allow_filesystem_write: false };
    ...
    fn allowed_operations_pass_policy_check() {
        let policy = SandboxPolicy { max_execution_ms: 1000, allow_network: true, allow_filesystem_write: true };
    ...
    fn sandboxed_subprocess_timeout_enforced() {
        ...
        let policy = SandboxPolicy { max_execution_ms: 50, allow_network: false, allow_filesystem_write: false };
...
impl Sandbox for SubprocessSandbox {
    fn execute(
        ...
        loop {
            match child.try_wait().map_err(|e| ToolError::ExecutionFailed(e.to_string()))? {
                Some(_status) => {
                ...
                None if start.elapsed() >= deadline => {
                    ...
                    return Err(ToolError::ExecutionFailed(
                        format!("sandbox timeout after {}ms", policy.max_execution_ms)
                    ));
                }
