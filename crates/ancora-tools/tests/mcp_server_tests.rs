...
impl Tool for MathTool {
    ...
    fn description(&self) -> &str { "multiplies two numbers" }
...
async fn spawn_server(registry: ToolRegistry, token: Option<&str>) -> (std::net::SocketAddr, oneshot::Sender<()>) {
...
async fn mcp_client_calls_ancora_tool() {
    ...
    assert!(response.contains("42"), "expected product 42 in: {}", response);
...
async fn mcp_server_rejects_unauthenticated_request() {
    ...
    assert!(response.starts_with("HTTP/1.1 401"), "expected 401, got: {}", &response[..60.min(response.len())]);
...
async fn mcp_server_accepts_authenticated_request() {
    ...
    assert!(response.starts_with("HTTP/1.1 200 OK"), "expected 200, got: {}", &response[..60.min(response.len())]);
    assert!(response.contains("multiply"));
