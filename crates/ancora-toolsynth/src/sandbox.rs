...
impl SandboxRunner {
    pub fn execute(spec: &ToolSpec, _input: &serde_json::Value) -> Result<serde_json::Value, SynthError> {
        match spec.effect_class {
            ...
            EffectClass::WriteExternal | EffectClass::Destructive => {
                Err(SynthError::SandboxViolation(
                    format!("effect {:?} not allowed in sandbox", spec.effect_class)
                ))
            }
        }
    }
