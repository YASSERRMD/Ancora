...
impl RedTeamObjective {
    ...
    pub fn start(&mut self) { self.status = ObjectiveStatus::InProgress; }
    pub fn achieve(&mut self, tick: u64) {
        self.status = ObjectiveStatus::Achieved;
        self.achieved_tick = Some(tick);
    }
    pub fn fail(&mut self) { self.status = ObjectiveStatus::Failed; }
    pub fn is_achieved(&self) -> bool { self.status == ObjectiveStatus::Achieved }
    pub fn is_done(&self) -> bool { self.status != ObjectiveStatus::Pending && self.status != ObjectiveStatus::InProgress }
}
...
impl ObjectiveTracker {
    pub fn new() -> Self { Self { objectives: Vec::new() } }
    pub fn add(&mut self, obj: RedTeamObjective) { self.objectives.push(obj); }
    pub fn count(&self) -> usize { self.objectives.len() }
    pub fn achieved_count(&self) -> usize { self.objectives.iter().filter(|o| o.is_achieved()).count() }
    pub fn pending_count(&self) -> usize { self.objectives.iter().filter(|o| o.status == ObjectiveStatus::Pending).count() }
    pub fn for_scenario<'a>(&'a self, scenario_id: &str) -> Vec<&'a RedTeamObjective> {
        self.objectives.iter().filter(|o| o.scenario_id == scenario_id).collect()
    }
