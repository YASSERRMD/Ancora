...
fn main() {
    ...
    sc.start();
    store.insert(sc);
    audit.record(RedTeamAuditEntry::new(1, "tenant-acme", "sc-1", RedTeamAction::ScenarioStarted, "redteam-op", "scenario started"));

    let mut attacks = AttackLog::new();
    attacks.record(AttackStep::new("a1", "sc-1", "SMB Relay", AttackVector::Network, AttackOutcome::Success, "T1557", "Captured NTLM hash", 2));
    attacks.record(AttackStep::new("a2", "sc-1", "Pass the Hash", AttackVector::Network, AttackOutcome::Detected, "T1550", "EDR alert raised", 3));
    audit.record(RedTeamAuditEntry::new(2, "tenant-acme", "sc-1", RedTeamAction::AttackStepExecuted, "redteam-op", "step a1 executed"));

    let mut detections = DetectionLog::new();
    detections.record(DetectionEvent::new("d1", "sc-1", DetectionSource::Edr, "Credential access detected", 3, true));

    let mut objectives = standard_objectives("sc-1");
    objectives.get_mut("obj-1").unwrap().achieve(2);
    audit.record(RedTeamAuditEntry::new(4, "tenant-acme", "sc-1", RedTeamAction::ObjectiveAchieved, "redteam-op", "obj-1 achieved"));

    store.get_mut("sc-1").unwrap().complete(10);
    audit.record(RedTeamAuditEntry::new(10, "tenant-acme", "sc-1", RedTeamAction::ScenarioCompleted, "redteam-op", "scenario complete"));
    ...
    println!("Objectives:        {}/{}", report.achieved_objectives, report.total_objectives);
