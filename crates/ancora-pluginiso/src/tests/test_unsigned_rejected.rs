use crate::signature::{
    PluginSignature, SignatureError, SignaturePolicy, SignatureVerifier, TrustedKey,
};

fn make_verifier_with_key(key_id: &str, key_bytes: Vec<u8>) -> SignatureVerifier {
    let mut v = SignatureVerifier::new();
    v.add_trusted_key(TrustedKey {
        key_id: key_id.into(),
...
fn unsigned_plugin_allowed_in_optional_mode() {
    let v = make_verifier_with_key("k1", vec![0xAA, 0xBB]);
    assert!(v
        .verify(b"plugin content", None, &SignaturePolicy::Optional)
        .is_ok());
}
...
fn unsigned_plugin_allowed_in_disabled_mode() {
    ...
    assert!(v
        .verify(b"plugin content", None, &SignaturePolicy::Disabled)
        .is_ok());
}
