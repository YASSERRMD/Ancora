...
fn test_pipeline_blocks_unredacted_pii() {
    ...
    let sample = ProdSample::new("s1", "m", "p", "my SSN is 123-45-6789", "sure", 100).with_pii();
...
fn test_safe_samples_excludes_nothing_after_redaction() {
    ...
    s1.redact("[X]");
    set.add(s1).unwrap();
    set.add(ProdSample::new("b", "m", "p", "clean prompt", "resp", 10))
        .unwrap();
    // Both are eval-safe.
...
fn test_redaction_overwrites_both_prompt_and_response() {
    let mut s =
        ProdSample::new("r", "m", "p", "sensitive prompt", "sensitive response", 5).with_pii();
    s.redact("[ANON]");
...
fn test_redaction_of_already_redacted_sample_is_noop() {
    ...
    let mut s = ProdSample::new("r2", "m", "p", "safe prompt", "safe response", 5).with_pii();
    s.redact("[A]");
    assert_eq!(s.sensitivity, Sensitivity::Redacted);
