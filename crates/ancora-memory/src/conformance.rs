...
pub mod suite {
    ...
    fn fresh(name: &str, dims: usize) -> MemStore {
        let s = MemStore::new();
        s.create_collection(CollectionSpec::new(name, dims, Distance::Cosine)).unwrap();
        s
    }

    fn pt(id: u64, v: Vec<f32>) -> Point { Point::new(id, v) }
    ...
    pub fn conformance_upsert_then_query_returns_nearest<S: VectorStore>(store: &S) {
        ...
        let r = store.query("col", QueryRequest::new(vec![1.0, 0.0, 0.0], 1)).unwrap();
        assert_eq!(r.len(), 1);
    ...
    pub fn conformance_metadata_filter_narrows_results<S: VectorStore>(store: &S) {
        ...
        let req = QueryRequest::new(vec![1.0, 0.0], 10)
            .with_filter(Filter::Eq("tag".to_owned(), PayloadValue::String("a".to_owned())));
        let r = store.query("col", req).unwrap();
        assert!(r.iter().all(|p| p.payload.get("tag") == Some(&PayloadValue::String("a".to_owned()))));
        assert_eq!(r.len(), 2);
    }
    ...
    pub fn conformance_delete_removes_a_point<S: VectorStore>(store: &S) {
        store.upsert("col", vec![pt(1, vec![1.0, 0.0]), pt(2, vec![0.0, 1.0])]).unwrap();
        store.delete("col", vec![PointId::Num(1)]).unwrap();
        let r = store.query("col", QueryRequest::new(vec![1.0, 0.0], 10)).unwrap();
    ...
    pub fn conformance_distance_metrics_behave<S: VectorStore>(store: &S) {
        ...
        let r = store.query("col", QueryRequest::new(vec![1.0, 0.0], 2)).unwrap();
    ...
    pub fn conformance_pagination_stable<S: VectorStore>(store: &S) {
        ...
        let page0 = store.query("col", QueryRequest::new(vec![1.0, 0.0, 0.0], 2).with_offset(0)).unwrap();
        let page1 = store.query("col", QueryRequest::new(vec![1.0, 0.0, 0.0], 2).with_offset(2)).unwrap();
    ...
    pub fn conformance_hybrid_search_merges_dense_and_keyword<S: VectorStore>(store: &S) {
        ...
        assert_eq!(r[0].id, PointId::Num(1), "apple+dense should rank id=1 first");
    }
    ...
    pub fn conformance_score_threshold_filters<S: VectorStore>(store: &S) {
        ...
        let r = store.query("col", req).unwrap();
        assert!(r.iter().all(|p| p.score >= 0.9), "all results must be above threshold");
    }
