...
mod milvus_hybrid_tests {
    ...
    fn hybrid_search_body_uses_rrf_reranker() {
        let body = hybrid_search_body(
            "docs", &[0.1f32; 4], "sparse", &[], 5, metric_type::L2,
        );
    ...
    fn hybrid_search_body_respects_top_k() {
        let body = hybrid_search_body(
            "docs", &[0.1f32; 4], "sparse", &[], 42, metric_type::COSINE,
        );
    ...
    fn hybrid_search_dense_sub_request_uses_embedding_field() {
        let body = hybrid_search_body(
            "docs", &[0.1f32; 4], "sparse", &[(0, 0.9f32)], 5, metric_type::COSINE,
        );
    ...
    fn hybrid_search_sparse_sub_request_uses_named_field() {
        let body = hybrid_search_body(
            "docs", &[0.1f32; 4], "my_sparse", &[(0, 0.9f32)], 5, metric_type::COSINE,
        );
    ...
    fn hybrid_search_outputs_payload_field() {
        ...
        assert!(fields.iter().any(|f| f == "payload"), "outputFields: {fields:?}");
    }
}
