...
mod hybrid_tests {
    ...
    fn hybrid_alpha_one_weights_vector_fully() {
        ...
        assert!(sql.contains("1 *") || sql.contains("1.0 *") || sql.contains("0.0 *"), "SQL: {sql}");
    }
    ...
    fn hybrid_alpha_half_has_equal_weights() {
        ...
        assert!(count >= 2, "expected at least two 0.5 occurrences, SQL: {sql}");
    }
    ...
    fn hybrid_with_filter_has_where_before_order_by() {
        ...
        assert!(where_pos < order_pos, "WHERE must precede ORDER BY, SQL: {sql}");
    }
    ...
    fn hybrid_column_sql_uses_specified_key() {
        ...
        assert!(!sql.contains("'text'"), "should not use default key, SQL: {sql}");
    }
    ...
    fn hybrid_query_always_has_order_by_score_desc() {
        for alpha in [0.0f32, 0.3, 0.5, 0.7, 1.0] {
            ...
            assert!(sql.contains("ORDER BY score DESC"), "alpha={alpha}, SQL: {sql}");
        }
    }
