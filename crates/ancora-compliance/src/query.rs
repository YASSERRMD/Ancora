...
impl ControlQuery {
    ...
    pub fn status(mut self, s: ControlStatus) -> Self {
        self.status = Some(s);
        self
    }
    pub fn has_evidence(mut self, v: bool) -> Self {
        self.has_evidence = Some(v);
        self
    }

    pub fn run<'a>(
        &self,
        controls: impl Iterator<Item = &'a ComplianceControl>,
    ) -> Vec<&'a ComplianceControl> {
        controls
            .filter(|c| {
