...
impl AttributeValue {
    pub fn as_str(&self) -> Option<&str> {
        match self { AttributeValue::String(s) => Some(s.as_str()), _ => None }
    }
    pub fn as_int(&self) -> Option<i64> {
        match self { AttributeValue::Integer(i) => Some(*i), _ => None }
    }
    pub fn as_bool(&self) -> Option<bool> {
        match self { AttributeValue::Bool(b) => Some(*b), _ => None }
    }
    pub fn contains(&self, val: &str) -> bool {
        match self { AttributeValue::List(l) => l.iter().any(|s| s == val), _ => false }
    }
}
...
impl AttributeSet {
    ...
    pub fn get(&self, key: &str) -> Option<&AttributeValue> { self.attrs.get(key) }
