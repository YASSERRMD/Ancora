...
fn main() {
    ...
    let usb_req = TransferRequest::new(
        "tx-001", "tenant-1", "alice",
        MediaType::UsbDrive, TransferDirection::Inbound,
        "firmware update", 1,
    ).with_checksum("sha256:deadbeef");

    let bluetooth_req = TransferRequest::new(
        "tx-002", "tenant-1", "bob",
        MediaType::Bluetooth, TransferDirection::Inbound,
        "file sync", 2,
    );
    ...
    boundary.add_zone(AirGapZone::new("pub-zone", "Public Zone", ZoneClassification::Public, "tenant-1"));
    ...
    audit.record(AirGapAuditEntry::new(1, "tenant-1", AirGapAction::TransferRequested, "alice", "usb import"));
    audit.record(AirGapAuditEntry::new(2, "tenant-1", AirGapAction::MediaBlocked, "sys", "bluetooth blocked"));

    let report = AirGapReport::generate(&boundary, &store, &audit, 100);
    println!("Report: zones={} restricted={} transfers={} pending={} audit={}",
        report.total_zones, report.restricted_zones,
        report.total_transfers, report.pending_transfers, report.total_audit_entries);
