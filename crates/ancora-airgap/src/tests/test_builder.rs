...
fn transfer_builder_defaults() {
    let req = TransferBuilder::new("r1", "t1", "alice", MediaType::UsbDrive, TransferDirection::Inbound).build();
...
fn transfer_builder_with_checksum() {
    let req = TransferBuilder::new("r1", "t1", "alice", MediaType::CdRom, TransferDirection::Outbound)
        .tick(100)
