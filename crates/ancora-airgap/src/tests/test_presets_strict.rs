...
fn strict_blocks_outbound() {
    ...
    let req = TransferRequest::new("r1", "t1", "alice", MediaType::PrintedDocument, TransferDirection::Outbound, "", 1)
...
fn strict_blocks_network_share() {
    let policy = strict_airgap_policy("t1");
    let req = TransferRequest::new("r1", "t1", "alice", MediaType::NetworkShare, TransferDirection::Inbound, "", 1)
        .with_checksum("sha256:abc");
    assert!(matches!(policy.evaluate(&req), PolicyVerdict::Deny(_)));
}
...
fn strict_requires_checksum_on_usb() {
    let policy = strict_airgap_policy("t1");
    let req = TransferRequest::new("r1", "t1", "alice", MediaType::UsbDrive, TransferDirection::Inbound, "", 1);
    assert!(matches!(policy.evaluate(&req), PolicyVerdict::Deny(_)));
}
