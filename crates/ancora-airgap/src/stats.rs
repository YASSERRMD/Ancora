...
impl AirGapStats {
    pub fn for_tenant(transfers: &[&TransferRequest], tenant_id: &str) -> Self {
        ...
        let pending = tenant_transfers
            .iter()
            .filter(|r| r.status == TransferStatus::Pending)
            .count();
        let approved = tenant_transfers
            .iter()
            .filter(|r| r.status == TransferStatus::Approved)
            .count();
        let rejected = tenant_transfers
            .iter()
            .filter(|r| r.status == TransferStatus::Rejected)
            .count();
        let completed = tenant_transfers
            .iter()
