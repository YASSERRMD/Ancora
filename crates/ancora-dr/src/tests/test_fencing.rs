#[cfg(test)]
mod tests {
    use crate::{FailoverController, JournalEntry, JournalStore, replicate};
    ...
    fn fencing_prevents_double_primary() {
        ...
        let err = p.append(JournalEntry { seq: 2, data: "split".into() });
    ...
    fn secondary_unfenced_on_promotion() {
        ...
        assert!(s.append(JournalEntry { seq: 2, data: "new".into() }).is_ok());
    }
}
