...
fn main() {
    ...
    let custom = IndicatorBuilder::new("custom-1", "acme", IndicatorKind::Domain, "malware-c2.example")
    ...
    for ind in [&bad_ip, &malware, &custom] {
        ...
        audit.record(ThreatIntelAuditEntry::new(1000, "acme", ThreatIntelAction::IndicatorAdded, &ind.id, "ingested"));
    }
    ...
    for ind in store.for_tenant("acme") {
        ...
        println!("Indicator {} score={:.1} level={} decision={:?}", ind.id, score.raw_score, score.level, decision);
    }

    let all_inds: Vec<&_> = store.for_tenant("acme");
    let summary = ThreatIntelSummary::generate(&all_inds, &alerts, "acme");
    println!("Summary: total={} active={} critical={} healthy={}", summary.total_indicators, summary.active_indicators, summary.critical_count, summary.is_healthy);
