...
fn score_critical_high_raw() {
    let i = Indicator::new("i1", "t1", IndicatorKind::IpAddress, "x", ThreatLevel::Critical, "f", 0);
...
fn score_low_threat_low_raw() {
    let i = Indicator::new("i1", "t1", IndicatorKind::Domain, "x", ThreatLevel::Informational, "f", 0);
    let score = ThreatScorer::score(&i, 0, 1000);
...
fn score_with_tags_higher_confidence() {
    let i = Indicator::new("i1", "t1", IndicatorKind::IpAddress, "x", ThreatLevel::High, "f", 0)
        .with_tag("apt");
    let score = ThreatScorer::score(&i, 0, 1000);
