...
fn alert_acknowledge() {
    let mut a = ThreatAlert::new("a1", "t1", "i1", ThreatLevel::Medium, "Suspicious domain", 1);
    a.acknowledge();
...
fn alert_store_open() {
    ...
    if let Some(a) = store.get_mut("a2") { a.close(); }
...
fn alert_store_for_tenant() {
    let mut store = AlertStore::new();
    store.add(ThreatAlert::new("a1", "t1", "i1", ThreatLevel::High, "M", 1));
    store.add(ThreatAlert::new("a2", "t2", "i2", ThreatLevel::High, "M", 2));
