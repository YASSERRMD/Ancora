use ancora_memcon::{
    ...
    EpisodicToSemanticPromoter, ForgettingPolicy, RetrievalChecker, SalienceItem, SalienceScorer,
    SummarizationPolicy, TokenBudget, Turn,
};
...
pub fn run_long_running_memory_example() {
    ...
    let episodic = vec![
        ...
        EpisodicEntry {
            key: "pref-offline".into(),
            content: "user wants offline".into(),
            occurrences: 1,
        },
    ];
    ...
    let _before_tokens =
        TokenBudget::total_tokens(&turns.iter().map(|t| t.content.clone()).collect::<Vec<_>>());
    let _after_tokens = TokenBudget::total_tokens(&retained_contents);

    assert!(
        quality_ok,
        "retrieval quality check failed post-consolidation"
    );
