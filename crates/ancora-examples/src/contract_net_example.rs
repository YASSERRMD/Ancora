use ancora_coord::{
    Blackboard, Bid, ContractNet, Auction, Negotiation, Proposal,
    Claim, ConflictPolicy, ConflictResolver, DeadlockDetector,
    AgentContract, CoordJournal,
};
...
pub fn run_contract_net_example() {
    ...
    let bids = vec![
        Bid { agent_id: "a".into(), task_id: "t1".into(), score: 0.6 },
        Bid { agent_id: "b".into(), task_id: "t1".into(), score: 0.9 },
    ];
    let winner = ContractNet::assign(&bids).unwrap();
    assert_eq!(winner.agent_id, "b");

    let mut auction = Auction::new("task-2");
    auction.submit(Bid { agent_id: "x".into(), task_id: "task-2".into(), score: 0.7 });
    auction.submit(Bid { agent_id: "y".into(), task_id: "task-2".into(), score: 0.5 });
    assert_eq!(auction.resolve().unwrap().agent_id, "x");

    let mut neg = Negotiation::new(3);
    neg.submit(Proposal { agent_id: "a".into(), value: 8 }).unwrap();
    neg.submit(Proposal { agent_id: "b".into(), value: 12 }).unwrap();
    assert_eq!(neg.consensus(), Some(10));

    let claims = vec![
        Claim { agent_id: "p".into(), priority: 5, arrived_at: 1 },
        Claim { agent_id: "q".into(), priority: 9, arrived_at: 2 },
    ];
