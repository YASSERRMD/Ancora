...
use ancora_reason::{ReasoningJournal, ReasoningEvent, StepDecomposer, StepVerifier};
use ancora_ageval::{EvalReport, MetricScore, GuardrailMetric, ReasoningMetric};
...
pub fn run() {
    ...
    let claims: Vec<String> = tasks.iter()
    ...
    for step in steps.iter_mut() {
        reason_journal.record(step.index as u64, ReasoningEvent::StepAdded {
            index: step.index,
        ...
        if result.passed {
            verified += 1;
            reason_journal.record(step.index as u64, ReasoningEvent::StepVerified { index: step.index });
        }
    }

    // Step 4: eval report
    let mut report = EvalReport::new("advanced-pipeline", 1);
    report.add_score(MetricScore::new("guardrail_catch_rate", GuardrailMetric::score(blocked, total)));
    report.add_score(MetricScore::new("reasoning_correctness", ReasoningMetric::score(verified, steps.len())));
