...
fn turns(n: usize) -> Vec<Turn> {
    (0..n).map(|i| Turn { index: i, role: "user".into(), content: format!("t{i}") }).collect()
}

fn sal_item(key: &str, imp: u32) -> SalienceItem {
    SalienceItem { key: key.into(), content: key.into(), importance: imp, access_count: 1, age_secs: 0 }
}
...
fn job_runs_and_journals_summarize_event() {
    ...
    assert!(output.summary.dropped_count > 0 || !output.summary.summary.is_empty() || output.summary.kept.len() == 1);
...
fn job_promotes_recurring_episodic_facts() {
    ...
    let mut journal = ConsolidationJournal::default();
    let episodic = vec![
        EpisodicEntry { key: "fact".into(), content: "recurring".into(), occurrences: 3 },
    ];
...
fn job_deduplicates_salience_items() {
    ...
    let has_dup = {
        ...
        k.sort();
        let deduped_len = { let mut k2 = k.clone(); k2.dedup(); k2.len() };
        k.len() != deduped_len
    };
    assert!(!has_dup);
}
