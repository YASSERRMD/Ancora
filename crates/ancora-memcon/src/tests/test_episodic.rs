...
fn promotion_moves_recurring_facts() {
    ...
    let entries = vec![
        EpisodicEntry { key: "a".into(), content: "fact-a".into(), occurrences: 5 },
        EpisodicEntry { key: "b".into(), content: "fact-b".into(), occurrences: 1 },
    ];
...
fn below_threshold_not_promoted() {
    ...
    let entries = vec![
        EpisodicEntry { key: "x".into(), content: "c".into(), occurrences: 5 },
    ];
...
fn exact_threshold_is_promoted() {
    let promoter = EpisodicToSemanticPromoter::new(3);
    let entries = vec![
        EpisodicEntry { key: "z".into(), content: "c".into(), occurrences: 3 },
    ];
