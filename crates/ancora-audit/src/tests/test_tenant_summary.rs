...
fn summary_groups_by_tenant() {
    ...
    log.append(AuditEntry::new(0, 2, "t2", "bob", "read", "r", Outcome::Failure, Severity::Warning));
    log.append(AuditEntry::new(0, 3, "t1", "alice", "write", "r", Outcome::Success, Severity::Info));
...
fn summary_sorted_by_tenant_id() {
    let mut log = ImmutableAuditLog::new();
    log.append(AuditEntry::new(0, 1, "zzz", "a", "op", "r", Outcome::Success, Severity::Info));
    log.append(AuditEntry::new(0, 2, "aaa", "b", "op", "r", Outcome::Success, Severity::Info));
    let entries: Vec<&AuditEntry> = log.entries().collect();
    let summaries = summarize_by_tenant(&entries);
