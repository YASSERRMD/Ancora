...
fn query_by_tenant_and_subject() {
    ...
    log.append(AuditEntry::new(0, 2, "t2", "alice", "write", "r", Outcome::Success, Severity::Info));
    log.append(AuditEntry::new(0, 3, "t1", "bob", "read", "r", Outcome::Success, Severity::Info));
    let results = AuditQuery::new().tenant("t1").subject("alice").run(log.entries());
...
fn query_by_outcome_filters() {
    ...
    log.append(AuditEntry::new(0, 3, "t1", "c", "op", "r", Outcome::Failure, Severity::Error));
    let results = AuditQuery::new().outcome(Outcome::Failure).run(log.entries());
