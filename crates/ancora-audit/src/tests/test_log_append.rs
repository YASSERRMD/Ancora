...
fn append_returns_sequential_ids() {
    ...
    let e1 = AuditEntry::new(0, 1, "t1", "alice", "read", "res", Outcome::Success, Severity::Info);
    let e2 = AuditEntry::new(0, 2, "t1", "alice", "write", "res", Outcome::Success, Severity::Info);
...
fn append_assigns_id_to_entry() {
    let mut log = ImmutableAuditLog::new();
    let e = AuditEntry::new(0, 10, "t1", "bob", "delete", "file", Outcome::Failure, Severity::Warning);
    let id = log.append(e);
